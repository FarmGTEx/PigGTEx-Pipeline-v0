...
if args.output_zscore:
    Metasoft_df = pd.DataFrame(0, index=df.index, columns=[j for i in sample_ids for j in [i+'_slope', i+'_slope_se', i+'_zval']], dtype=np.float32)
    Metasoft_df[sample_ids[0]+'_slope'] = df['slope']
    Metasoft_df[sample_ids[0]+'_slope_se'] = df['slope_se']
    Metasoft_df[sample_ids[0]+'_zval'] = df['slope']/df['slope_se']
elif args.only_zscore:
    Metasoft_df = pd.DataFrame(0, index=df.index, columns=[j for i in sample_ids for j in [i+'_zval']], dtype=np.float32)
    Metasoft_df[sample_ids[0]+'_zval'] = df['slope']/df['slope_se']
else:
...
for k,(i,p) in enumerate(zip(sample_ids[1:], paths[1:])):
    ...
    if args.output_zscore:
        ...
        Metasoft_df[i+'_zval'] = df['slope']/df['slope_se']
    elif args.only_zscore:
        Metasoft_df[i+'_zval'] = df['slope']/df['slope_se']
    else:
        Metasoft_df[i+'_slope'] = df['slope']
...
if args.dropna:
    Metasoft_df.dropna(axis = 0, inplace = True)
    
if args.subset is not None:
    nr = [i for i in range(Metasoft_df.shape[0])]
    random.seed(args.seed)
    subset_nr = random.sample(nr,args.subset)
    subset_nr.sort()
    Metasoft_df = Metasoft_df.iloc[subset_nr,:]
...
if args.chunks is not None:
    ...
    for i in np.arange(args.chunks):
        ...
        with gzip.open(os.path.join(args.output_dir, args.prefix+'.chunk{:03d}.txt.gz'.format(i)), 'wt', compresslevel=1) as f:
            Metasoft_df.iloc[i*chunk_size:(i+1)*chunk_size].to_csv(f, sep='\t', float_format='%.6g', na_rep='NA')
    print()

if args.chunks is None:
    print('Writing full table')
    with gzip.open(os.path.join(args.output_dir, args.prefix+'.txt.gz'), 'wt', compresslevel=1) as f:
