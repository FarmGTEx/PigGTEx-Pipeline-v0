...
with open(input_file) as f:
    ...
    if header[0]=='pair_id':
        subprocess.check_call('tail -n+2 {0} > {0}.tmp && mv {0}.tmp {0}'.format(input_file), shell=True, executable='/bin/bash')
...
if args.metasoft_input.endswith('.gz'):
    subprocess.check_call('rm -rf '+input_file, shell=True, executable='/bin/bash')
subprocess.check_call('gzip -1 '+os.path.join(args.output_dir, args.prefix+'.metasoft.txt'), shell=True, executable='/bin/bash')
